...
export default class RichTextEditorView extends View {
    constructor() {
        ...
        this.container.append(el("section.markdown-editor-section", el("h2", "Markdown Editor"), markdownEditor = new MarkdownEditor(), el("h3", "Preview (HTML)"), markdownDocument = el(".markdown-document")), el("section.rich-text-editor-section", el("h2", "Rich Text Editor"), richTextEditor = new RichTextEditor(), el("h3", "Preview (Markdown)"), richTextDocument = el(".markdown-source")));
        markdownEditor.on("contentChanged", async (newContent) => {
            markdownDocument.htmlElement.innerHTML = await MarkdownConverter
                .convertMarkdownToHtml(newContent);
        });
        richTextEditor.on("contentChanged", async (newContent) => {
